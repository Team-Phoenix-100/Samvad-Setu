  ...
  return (
    <div className="w-64 flex-shrink-0 bg-[#16272B] border-r border-[#233E44] flex flex-col">
      <div className="p-6 border-b border-[#233E44]">
        <h1 className="text-2xl font-bold font-display text-primary-custom">Samvad<span className="text-accent-primary">Setu</span></h1>
        <p className="text-xs font-mono text-accent-secondary mt-1">DHTE Admin Panel</p>
      </div>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center justify-between px-3 py-2.5 rounded-lg transition-colors text-sm font-medium ${
                isActive 
                  ? 'bg-[#E8A33D]/10 text-accent-primary border border-[#E8A33D]/20' 
                  : 'text-muted-custom hover:bg-[#233E44]/50 hover:text-primary-custom'
              }`
            }
          >
            ...
            {link.badge > 0 && (
              <span className="bg-[#C1443B] text-primary-custom text-[10px] font-bold px-2 py-0.5 rounded-full">
                {link.badge}
              </span>
            )}
          </NavLink>
      ...
      <div className="p-4 border-t border-[#233E44]">
        <button className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-accent-urgent hover:bg-[#C1443B]/10 rounded-lg w-full transition-colors">
          <LogOut size={18} />
          Sign Out
        </button>
      </div>
