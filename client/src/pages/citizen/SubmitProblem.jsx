...
export default function SubmitProblem() {
  ...
  return (
    <div className="min-h-screen bg-base text-primary-custom p-4 sm:p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="space-y-2 border-b border-surface-raised pb-4">
        <span className="text-xs font-mono text-accent-primary uppercase">Phase 3 • Step {step} of 5</span>
      ...
      {step === 1 && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="bg-surface p-5 sm:p-6 rounded-xl border border-surface-raised space-y-4">
          ...
          <div className="space-y-1">
            <label className="text-xs font-mono text-muted-custom">Problem Title *</label>
            <input
              type="text"
              required
              ...
              className="w-full bg-base border border-surface-raised rounded-lg p-3 text-sm focus:outline-none focus:border-[#E8A33D]"
            />
          </div>
      ...
      {step === 2 && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="bg-surface p-5 sm:p-6 rounded-xl border border-surface-raised space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-mono text-muted-custom">District</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="text-xs font-mono text-muted-custom">Block / Locality</label>
              <input
                type="text"
          ...
          <div className="p-6 border border-dashed border-surface-raised rounded-lg bg-base text-center space-y-3">
            <MapPin className="mx-auto text-accent-primary" size={32} />
            <p className="text-xs text-muted-custom">Click the map or drag the marker to choose the problem location.</p>
            <div className="h-52 overflow-hidden rounded-lg border border-surface-raised text-left">
              <MapContainer
            ...
            <span className="block text-xs font-mono text-accent-secondary bg-[#2F9E8F]/10 px-2 py-1 rounded w-max mx-auto mb-2">
              GPS Lat: {formData.lat.toFixed(4)}, Lng: {formData.lng.toFixed(4)}
            </span>
            {isResolvingLocation && <p className="text-xs text-accent-primary">Finding district and locality...</p>}
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="bg-surface p-5 sm:p-6 rounded-xl border border-surface-raised space-y-4">
          <div className="flex justify-between items-center">
            ...
            {formData.imageUploaded && (
              <span className="text-xs font-mono text-accent-secondary bg-[#2F9E8F]/15 px-2.5 py-1 rounded-full flex items-center gap-1">
          ...
          {formData.imageUploaded && formData.previewUrls.length > 0 ? (
            ...
            <div className="bg-base border border-surface-raised rounded-xl p-4 space-y-4">
              <div className="relative group overflow-hidden rounded-lg border border-surface-raised bg-black/40 flex items-center justify-center h-48 sm:h-56">
                <img
                ...
                <button
                  ...
                  className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 text-primary-custom font-medium text-xs backdrop-blur-xs cursor-pointer"
                >
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-surface-raised">
                <div className="text-xs text-muted-custom">
                  <span className="text-accent-secondary font-semibold">Photo Attached</span> • Ready for submission
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-surface-raised hover:bg-[#E8A33D] hover:text-[#0F1B1E] text-primary-custom rounded-lg transition-colors cursor-pointer"
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-surface-raised hover:bg-[#2F9E8F] hover:text-primary-custom text-primary-custom rounded-lg transition-colors cursor-pointer"
            ...
            >
              {isProcessingImage ? (
                <div className="space-y-2 py-4">
                  ...
                  <p className="text-sm text-accent-primary">Optimizing photo for submission...</p>
                </div>
              ) : (
                <>
                  <div className="w-14 h-14 rounded-full bg-surface border border-surface-raised flex items-center justify-center mx-auto text-accent-primary">
                  ...
                  <span className="inline-block text-[11px] font-mono text-accent-secondary bg-[#2F9E8F]/10 px-2 py-1 rounded">
          ...
          <div className="border-t border-surface-raised pt-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-semibold flex items-center gap-2"><Mic size={16} className="text-accent-secondary" /> Add a voice note</p>
                <p className="text-xs text-muted-custom">Whisper transcription will enrich the triage brief.</p>
              </div>
              <button type="button" onClick={toggleVoiceRecording} className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-semibold ${isRecording ? 'bg-[#C1443B] text-primary-custom' : 'bg-[#2F9E8F] text-[#0F1B1E]'}`}>
                {isRecording ? <Square size={14} /> : <Mic size={14} />}
                {isRecording ? `Stop ${recordingSeconds}s` : audioFile ? 'Record again' : 'Record voice'}
              </button>
            </div>
            {audioFile && !isRecording && <p className="text-xs text-accent-secondary flex items-center gap-2"><CheckCircle size={13} /> Voice note ready: {audioFile.name}</p>}
          </div>
        </motion.div>
      )}
      ...
      {step === 4 && (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="bg-surface p-5 sm:p-6 rounded-xl border border-[#2F9E8F] space-y-4">
          <div className="flex items-center gap-2 text-accent-secondary">
            <Sparkles size={20} />
            <h2 className="text-lg font-bold font-display">4. AI Analysis & Routing Preview</h2>
          </div>

          <div className="p-4 bg-base rounded-lg border border-surface-raised space-y-3 text-xs">
            <div className="flex justify-between">
              <span className="text-muted-custom">Detected Category:</span>
              <span className="font-bold text-accent-primary">{aiClassification?.category || formData.category}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-custom">Calculated Urgency:</span>
              <span className="font-bold text-accent-secondary uppercase">{aiClassification?.severity || formData.urgency}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-custom">Recommended Department:</span>
              <span className="font-bold text-primary-custom">{aiClassification?.department || 'Public Works Department'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-custom">AI Confidence Score:</span>
              <span className="font-bold text-accent-primary">{aiClassification?.confidence ? `${Math.round(aiClassification.confidence * 100)}%` : 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-custom">Municipal SLA target:</span>
              <span className="font-bold text-accent-primary">{formData.urgency === 'critical' ? '24' : formData.urgency === 'urgent' || formData.urgency === 'high' ? '72' : '168'} hours</span>
            </div>
            {audioFile && <div className="flex items-center gap-2 text-accent-secondary border-t border-surface-raised pt-3"><Radio size={14} /> Voice note queued for Whisper transcription</div>}
          </div>
          <div className="p-3 rounded-lg border border-[#E8A33D]/30 bg-[#E8A33D]/10 text-xs text-accent-primary flex gap-2"><MapPin size={15} className="shrink-0" /> Spatial check: a nearby report within 150m may be fused into the existing master ticket after server verification.</div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3 }} className="bg-surface p-5 sm:p-6 rounded-xl border border-surface-raised space-y-4">
          ...
          <div className="p-4 bg-base rounded-lg space-y-3 text-xs border border-surface-raised">
            ...
            {formData.description && (
              <p><strong className="text-muted-custom">Description:</strong> {formData.description}</p>
            )}
            ...
            {formData.previewUrls.length > 0 && (
              <div className="pt-2 border-t border-surface-raised flex items-center gap-3">
                <img
                  src={formData.previewUrls[0]}
                  alt="Proof preview"
                  className="w-14 h-14 object-cover rounded-lg border border-surface-raised"
                />
                <div>
                  <span className="text-accent-secondary font-semibold block">Photo Evidence Attached</span>
                  <span className="text-[11px] text-muted-custom">Verified proof ready for submission</span>
                </div>
              </div>
            )}
            {audioFile && <p className="pt-2 border-t border-surface-raised text-accent-secondary"><Mic size={14} className="inline mr-2" />Voice note attached for Whisper</p>}
          </div>
        ...
        {cropModalOpen && (
          ...
          >
            <motion.div 
              ...
              className="bg-surface w-full max-w-lg rounded-2xl border border-surface-raised overflow-hidden flex flex-col my-auto max-h-[90vh]"
            >
              {/* Modal Header */}
              <div className="p-4 border-b border-surface-raised flex justify-between items-center bg-base shrink-0">
                <div>
                  <h3 className="font-bold text-primary-custom text-base">Crop & Adjust Image</h3>
                  <p className="text-[11px] text-muted-custom">Optional: Zoom and position your photo</p>
                </div>
                <button
                  type="button"
                  onClick={() => setCropModalOpen(false)}
                  className="text-muted-custom hover:text-red-400 p-1 cursor-pointer"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="p-4 space-y-3 overflow-y-auto">
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-xs font-mono text-muted-custom">Zoom</label>
                    <span className="text-xs font-mono text-accent-primary">{zoom.toFixed(1)}x</span>
                  </div>
                  <input
                    type="range"
                    value={zoom}
              ...
              <div className="p-3 sm:p-4 bg-base border-t border-surface-raised flex flex-wrap justify-end gap-2 shrink-0">
                <Button
                  variant="outline"
        ...
        {previewModalOpen && formData.previewUrls.length > 0 && (
          ...
          >
            <button
              type="button"
              onClick={() => setPreviewModalOpen(false)}
              className="absolute top-4 right-4 text-primary-custom hover:text-red-400 bg-black/50 rounded-full w-10 h-10 flex items-center justify-center backdrop-blur-sm z-50 cursor-pointer"
            >
              ✕
            </button>
            <motion.img 
